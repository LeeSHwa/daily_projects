...
ISBN = list(input())
weights = [1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3] # 가중치

index = ISBN.index('*')

ISBN.remove('*')
removed_ISBN = list(map(int, ISBN))
#-----여기까진 맞는 듯------#

if index == 12: # 만약 체크디지트가 *이라면?
    print(10 - (sum(x * y for x, y in zip(removed_ISBN, weights)))%10) # 바로 출력

else:
    check_digit = removed_ISBN.pop(-1)
    popped_wei = weights.pop(index)
    rest_sum = sum(x * y for x, y in zip(removed_ISBN, weights))    

    for i in range(10):
        # 만약 계산 결과가 만약 rest_sum + popped_wei * i 의 값이 10이라면 check_digit가 10이됨
        # 그것을 방지하기 위해 한 번 더 10 나머지 연산을 수행 (단, 0이면 checkdigit은 0이다를 활용)
        if check_digit == (10 - (rest_sum + popped_wei * i) % 10) % 10:
            print(i)
            break       